...
//NETRESOURCE structure wrapper used to save text data that freed by WNetEnumClose(_);
    ...
    public:
        ...
        m_nr(nr)
        {
            ...
            _tcscpy_s(m_nr.lpLocalName, (m_strLocalName.size() + 1), m_strLocalName.c_str());

            m_nr.lpRemoteName = new TCHAR [m_strRemoteName.size() + 1];
            _tcscpy_s(m_nr.lpRemoteName, (m_strRemoteName.size() + 1), m_strRemoteName.c_str());

            m_nr.lpComment = new TCHAR [m_strComment.size() + 1];
            _tcscpy_s(m_nr.lpComment,(m_strComment.size() + 1), m_strComment.c_str());

            m_nr.lpProvider = new TCHAR [m_strProvider.size() + 1];
            _tcscpy_s(m_nr.lpProvider, (m_strProvider.size() + 1), m_strProvider.c_str());
